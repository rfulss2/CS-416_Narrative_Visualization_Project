...
export function sceneContribSharePie(state) {
  ...
  // describe to the audience what this pie chart is
  svg.append('text')
    .attr('x', width/2)
    .attr('y', height - 20)
    ...
    .text('Breakdown of EU budget contributions by country (from 2014 to 2023).');

  const entries = Array.from(state.revSum.entries()).map(([country, paid]) => ({ country, paid }));
  const total = d3.sum(entries, d => d.paid);

  // figure out how to divide countries
  // could use a threshold, prob should base it on LV
  // 2% seems ok, REMEMBER - may need to finetune
  const division_threshold = 0.02;
  const big = [], small = [];
  for (const d of entries) {
    if (d.paid / total < division_threshold) {
      small.push(d);
    } else { 
      big.push(d);
    }
  ...
  // get all country names and abbreviationss - so can have column that explains everything
  const countryNames = {
